...
class Mega_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $args->settings['use-dropdown-icon'] && ( in_array( 'menu-item-has-children', $item->classes ) || $this->is_mega_enabled( $item->ID ) ) ) {
		...
		$link_html = sprintf(
			'%1$s<a%2$s>%3$s<div class="jet-mega-menu-item__title"><div class="jet-mega-menu-item__label">%4$s</div>%5$s</div>%6$s</a>%7$s',
			$args->link_before,
			$attributes,
			$icon,
			$title,
