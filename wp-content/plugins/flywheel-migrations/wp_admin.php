...
class FWLWPAdmin {
	...
	public function initHandler() {
		...
		if (array_key_exists('bvnonce', $_REQUEST) &&
			...
			FWLAccount::addAccount($this->settings, $keys[0], $keys[1]);
	...
	public function settingsLink($links, $file) {
		if ( $file == plugin_basename( dirname(__FILE__).'/flywheel.php' ) ) {
			$links[] = '<a href="'.$this->mainUrl().'">'.__( 'Settings' ).'</a>';
		}
		return $links;
	}

	public function fwlsecAdminMenu($hook) {
		if ($hook === 'toplevel_page_flywheel') {
			wp_register_style( 'fwlsurface', plugins_url( 'assets/css/style.css', __FILE__ ) );
			wp_enqueue_style('fwlsurface');
		}
	}
	...
	public function showErrors() {
		...
		if (array_key_exists('error', $_REQUEST)) {
			...
			$open_tag = '<div style="padding-bottom:0.5px; color:red;"><p>';
			$close_tag = '</p></div>';
			if ($error == "email") {
				echo $open_tag.'Incorrect Email.'.$close_tag;
			} else if (($error == "custom") && isset($_REQUEST['bvnonce']) && wp_verify_nonce($_REQUEST['bvnonce'], "bvnonce")) {
				echo $open_tag.esc_html(base64_decode($_REQUEST['message'])).$close_tag;
			}
		}
	}
	...
	public function siteInfoTags() {
		...
		$secret = FWLRecover::defaultSecret($this->settings);
		$public = FWLAccount::getApiPublicKey($this->settings);
	...
	public function activateWarning() {
		global $hook_suffix;
		if (!FWLAccount::isConfigured($this->settings) && $hook_suffix == 'index.php' ) {
?>
			<div id="message" class="updated" style="padding: 8px; font-size: 16px; background-color: #dff0d8">
						<a class="button-primary" href="<?php echo $this->mainUrl(); ?>">Activate Flywheel</a>
						&nbsp;&nbsp;&nbsp;<b>Almost Done:</b> Activate your Flywheel account to migrate your site.
			</div>
<?php
		}
	}
