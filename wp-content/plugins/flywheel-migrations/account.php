...
if (!class_exists('FWLAccount')) :
	class FWLAccount {
		...
		public function authenticatedUrl($method) {
			$bvinfo = new FWLInfo($this->settings);
		...
		public static function removeByAccountType($settings, $account_type) {
			$accounts = FWLAccount::accountsByType($settings, $account_type);
			if (sizeof($accounts) >= 1) {
				foreach ($accounts as $pubkey => $value) {
					FWLAccount::remove($settings, $pubkey);
				}
				return true;
			}
			return false;
		...
		public static function removeByAccountGid($settings, $account_gid) {
			$accounts = FWLAccount::accountsByGid($settings, $account_gid);
			if (sizeof($accounts) >= 1) {
				foreach ($accounts as $pubkey => $value) {
					FWLAccount::remove($settings, $pubkey);
				}
